...
function configureChannel(channel: RTCDataChannel, remoteId?: string) { if (remoteId) channels.set(remoteId, channel); channel.onmessage = event => { try { show(JSON.parse(event.data) as ChatMessage); } catch { /* Ignore malformed peer data. */ } }; }
async function onSignal(signal: Signal) {
  ...
  if (signal.type === 'offer') { const pc = await connectPeer(signal.target_peer_id, false); await pc.setRemoteDescription(signal.payload as RTCSessionDescriptionInit); const answer = await pc.createAnswer(); await pc.setLocalDescription(answer); sendSignal({ type: 'answer', target_peer_id: signal.target_peer_id, payload: answer }); }
  if (signal.type === 'answer') await peers.get(signal.target_peer_id)?.setRemoteDescription(signal.payload as RTCSessionDescriptionInit);
  if (signal.type === 'ice_candidate') await peers.get(signal.target_peer_id)?.addIceCandidate(signal.payload as RTCIceCandidateInit);
}

joinButton.onclick = async () => { joinButton.disabled = true; roomInput.disabled = true; nameInput.disabled = true; roomId = roomInput.value.trim() || 'general'; try { await initWasm(); stream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false }); socket = new WebSocket(signalingInput.value.trim()); socket.onopen = () => { connection.textContent = 'connected'; socket?.send(make_join(roomId, peerId)); }; socket.onmessage = event => onSignal(JSON.parse(event.data)); socket.onclose = () => { connection.textContent = 'offline'; }; messageInput.disabled = false; (composer.querySelector('button') as HTMLButtonElement).disabled = false; muteButton.disabled = false; leaveButton.disabled = false; } catch (error) { connection.textContent = error instanceof Error ? error.message : 'failed'; joinButton.disabled = false; } };
composer.onsubmit = event => { event.preventDefault(); const content = messageInput.value.trim(); if (!content) return; const message: ChatMessage = { sender_id: nameInput.value || peerId, content, timestamp: Date.now() }; for (const channel of channels.values()) if (channel.readyState === 'open') channel.send(JSON.stringify(message)); show(message); messageInput.value = ''; };
