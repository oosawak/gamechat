...
const messageInput = $('message') as HTMLInputElement, composer = $('composer') as HTMLFormElement, messages = $('messages'), connection = $('connection'), peersLabel = $('peers'), network = $('network'), audio = $('audio');
...
const updateNetwork = (state: string) => { network.textContent = `ICE: ${state}`; network.dataset.state = state; };
...
async function connectPeer(remoteId: string, initiator: boolean) {
  ...
  pc.ontrack = event => { const element = document.createElement('audio'); element.autoplay = true; element.dataset.peerId = remoteId; element.srcObject = event.streams[0]; audio.append(element); };
  pc.oniceconnectionstatechange = () => updateNetwork(pc.iceConnectionState);
  pc.onconnectionstatechange = () => {
    if (pc.connectionState === 'connected') updateNetwork('connected');
    if (pc.connectionState === 'failed' || pc.connectionState === 'closed') removePeer(remoteId);
  };
...
function removePeer(remoteId: string) {
  ...
  channels.get(remoteId)?.close();
  channels.delete(remoteId);
  pendingCandidates.delete(remoteId);
  audio.querySelectorAll(`[data-peer-id="${CSS.escape(remoteId)}"]`).forEach(element => element.remove());
  updatePeers();
  if (peers.size === 0) updateNetwork('idle');
}
async function onSignal(signal: Signal) {
  if (signal.type === 'joined') { for (const id of signal.peers ?? []) await connectPeer(id, true); return; }
  if (signal.type === 'peer_left') { removePeer(signal.peer_id!); return; }
...
joinButton.onclick = async () => { joinButton.disabled = true; roomInput.disabled = true; nameInput.disabled = true; roomId = roomInput.value.trim() || 'general'; try { await restoreHistory(); await initWasm(); stream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false }); socket = new WebSocket(signalingInput.value.trim()); socket.onopen = () => { connection.textContent = 'connected'; updateNetwork('checking'); socket?.send(make_join(roomId, peerId)); }; socket.onmessage = event => onSignal(JSON.parse(event.data)); socket.onclose = () => { connection.textContent = 'offline'; updateNetwork('offline'); }; messageInput.disabled = false; (composer.querySelector('button') as HTMLButtonElement).disabled = false; muteButton.disabled = false; leaveButton.disabled = false; } catch (error) { connection.textContent = error instanceof Error ? error.message : 'failed'; updateNetwork('error'); joinButton.disabled = false; roomInput.disabled = false; nameInput.disabled = false; } };
...
leaveButton.onclick = () => { [...peers.keys()].forEach(removePeer); stream?.getTracks().forEach(track => track.stop()); socket?.close(); location.reload(); };
