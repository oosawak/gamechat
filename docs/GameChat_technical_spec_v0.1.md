...
MVP は STUN のみを前提とし、TURN は後から追加可能な設定にする。接続失敗が確認された段階で TURN を導入する。
...
シグナリングサーバーのホスティング先、STUN / TURN 構成、Google Groups の具体名、認証トークン、推奨ルーム人数、ライセンス、共有履歴方式、SFU 方式は実装を進めながら決定する。
